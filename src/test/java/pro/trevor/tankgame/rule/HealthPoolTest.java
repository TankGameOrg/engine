...
public class HealthPoolTest {
    ...
    public void GainHealthInHealthPool() {
        Tank tank = TankBuilder.buildV3Tank().at(new Position("A1")).with(TankAttribute.DURABILITY, 2)
    ...
    public void HealthPoolTwoRegen() {
        Tank tank = TankBuilder.buildV3Tank().at(new Position("A1")).with(TankAttribute.DURABILITY, 1)
                .with(TankAttribute.DEAD, false).finish();
        DummyState state = new DummyState();
